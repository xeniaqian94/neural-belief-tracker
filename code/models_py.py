...
def model_definition(vector_dimension, label_count, slot_vectors, value_vectors, use_delex_features=False,
    ...
    hidden_layer_joint = tf.nn.sigmoid(tf.reshape(
        tf.matmul(h_utterance_representation_candidate_interaction, w_joint_hidden_layer) + b_joint_hidden_layer,
    ...
    y_presoftmax = tf.reshape(tf.matmul(tf.reshape(hidden_layer_joint_with_dropout, [-1, hidden_units_1]),
    ...
    if value_specific_decoder and False:
        h_utterance_representation_for_full_softmax = define_CNN_model(utterance_representations_full, num_filters,
    ...
    if use_softmax:
        ...
        if False and learn_belief_state_update:
            ...
            if value_specific_decoder:  # value-specific update
                ...
                y_combine = tf.matmul(y_past_state, W_memory) + tf.matmul(y_presoftmax,
                                                                          W_current)  # + tf.matmul(sysreq, W_current_req) + tf.matmul(sysconf, W_current_conf)
